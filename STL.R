...
for(file in files) {
  ...
  # visualize
  dataset$anomaly_STL <- ifelse(dataset$time %in% anomalies_STL$time, 1, 0)
  
  ggplot(dataset) + geom_line(aes(time, value)) +
    geom_point(data=dataset[dataset$anomaly_STL==1,], aes(time, value, color='forestgreen', size=1.5, alpha=0.05)) +  
    geom_point(data=dataset[dataset$label==1,], aes(time, value, color='firebrick1', size=1.5, alpha=0.05)) +
    theme(legend.position = "none")
  
  ggsave(paste0("./Plots/", unlist(strsplit(file, split='\\.'))[1],"-STL-", ".png"))
}
